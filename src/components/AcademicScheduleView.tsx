              ...
              >
                <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
                  <span className="bg-blue-100 text-blue-900 font-extrabold px-3 py-1 rounded-lg text-xs border border-blue-200">
                    📘 {(() => {
                      ...
                      if (!name) return 'Slot Terkunci';
                      if (name === 'Pengajian / Aktiviti' || name.startsWith('Pensyarah (') || name === 'TERKUNCI') {
                        const raw = slot.lecturerName || name.replace(/^Pensyarah\s*\((.*)\)$/, '$1') || name;
                ...
                <div>
                  <h4 className="text-base font-bold text-slate-900 leading-snug">
                    {(slot.courseCode === 'TERKUNCI' ? 'LOCKED' : slot.courseCode)} — {slot.courseName}
                  </h4>
